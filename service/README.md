...
2. 服务端拿到第三方流，对流的每帧进行计算（自然是多线程的处理与计算）：
   ...
   - 再次，如果不近似，则将**该帧送入** Encoder 进行编码，Encoder 应返回**残差图与原图相加完成的结果图**。然后，将该处理完毕的帧的 `base64` 编码存入当前线程的一个全局变量中。如若近似，跳过 Encoder ，直接将上一帧的处理后的图作为结果图送入视频流。
...
> 由于 RTMP 协议的延迟问题，这里摒弃了 Java。
 ...
 - **服务端（采用 Flask 框架）**
   ...
   - [x] 根据拉流活动情况将拓展信息持久化。
   - [x] 推流：`FFmpeg` 推流。
   - [x] 图像上传接口。（这里另外实现了 Encoder 部分的上传接口，可以同屏比较两图异同）
   - [x] 图像回传。（图片进入 Decoder 前的透视变换结果图回传到前端；Encoder 后的结果图回传到前端）
   - [x] 透视变换。
   - [x] 指定透视变换位置；可以在一张默认空白大图中嵌入处理后的透视图，或者把透视图缩放到指定尺寸。
   - [x] 查询数据库并返回信息。
